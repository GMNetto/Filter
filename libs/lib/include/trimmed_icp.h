...
namespace pcl
{
  namespace recognition
  {
    ...
    transform(const Eigen::Matrix<float,4,4>& mat, const T &p, T &out)
    {
    ...
    class PCL_EXPORTS TrimmedICP: public pcl::registration::TransformationEstimationSVD<PointT, PointT, Scalar>
    {
      ...
      public:
        ...
        align (const PointCloud& source_points, int num_source_points_to_use, Matrix4& guess_and_result) const
        {
          ...
          PointT transformed_source_point;
...
//          printf ("\nalign\n");
          ...
          do
          {
            //std::cout << "Trans: " << guess_and_result << std::endl;
            ...
            for ( int i = 0 ; i < num_source_points ; ++i )
            {
              ...
              transform (guess_and_result, source_points.points[i], transformed_source_point);

              //std::cout << "Trans point " << transformed_source_point << std::endl;
